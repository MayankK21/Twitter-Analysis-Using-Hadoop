...
6. Twitter API requires creation of twitter app and generation of consumer keys and access tokens. The steps to create Twitter API are as follows:
	...
	(a) You need to have a working Twitter account for the authentication purposes. Twitter doesn’t allow access to its data if one doesn’t have 		    twitter developer account. So, we need to fill out the details regarding the purpose that we need the data for.
